...
@Configuration
public class TenantLoyCoreDatabaseConfig {
    private final MetaDataSourceSearchService metaDataSourceSearchService;

    @Lazy
    @Bean(name= TenantDataSourceBeanName.LOYCORE_TENANT_DATASOURCE)
    public DataSource dataSource() throws Exception {
        log.info("============= tenant LoyCore dataSource Setting...  ==============");

        Map<String, TenantDatasourceVo> tenantDatasourceVoMap = metaDataSourceSearchService.searchTenantDatasource(DatabaseType.LOYCORE);
        ...
        log.info("============= tenant LoyCore dataSource Complete...  ==============");

        return dataSouceRouter;
    }

    @Bean(name = TenantDataSourceBeanName.DATASOURCE_LOY_CORE_TRANSACTION_MANAGER_FACTORY)
    public PlatformTransactionManager platformTransactionManager (@Qualifier(TenantDataSourceBeanName.LOYCORE_TENANT_DATASOURCE) DataSource dataSource) {
